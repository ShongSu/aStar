...
//    private final int COST_DIAGONAL = 14;//斜方向移动的路径评分
    ...
    //计算H值
    private void countH(int[][] map, Node node,Node eNode){
    	map=this.map;
    	int h1 = Math.abs(node.getX()-eNode.getX());
    	int h2 = Math.abs(node.getY()-eNode.getY());
    	...
    	if(node.getY()<eNode.getY()){
    		for(int i=node.getY()+1; i<=eNode.getY();i++){
    			System.out.println("横坐标:("+node.getX()+","+i+")" );
    			if(map[node.getX()][i]==0){
    				count1++;
    			}	
    		}
    	}
    	...
    	if(node.getX()<eNode.getX()){
    		for(int i=node.getX()+1; i<=eNode.getX();i++){
    			System.out.println("纵坐标:("+i+","+eNode.getY()+")" );
    			if(map[i][eNode.getY()]==0){
    				count1++;
    			}	
    		}
    	}
    	...
    	System.out.println("Test: nodeY="+node.getY());
    	System.out.println("Test: eNodeY="+eNode.getY());
    	System.out.println("Test: nodeX="+node.getX());
    	System.out.println("Test: eNodeX="+eNode.getX());

    	//先纵后横
    	if(node.getX()<eNode.getX()){
    		for(int i=node.getX()+1; i<=eNode.getX();i++){
    			System.out.println("2纵坐标:("+i+","+node.getY()+")" );
    			if(map[i][node.getY()]==0){
    				count2++;
    			}	
    		}
    	}
    	...
    	if(node.getY()<eNode.getY()){
    		for(int i=node.getY()+1; i<=eNode.getY();i++){
    			System.out.println("2横坐标:("+eNode.getX()+","+i+")" );
    			if(map[eNode.getX()][i]==0){
    				count2++;
    			}	
    		}
    	}
    	...
    	h3 = count1 < count2 ? count1 : count2;
